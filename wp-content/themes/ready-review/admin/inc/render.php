...
class ready_reviewRenderView {
    ...
    public function __construct() {
        $css_path = ready_review_config("admin_template_directory_uri") . "/css/";
        $js_path = ready_review_config("admin_template_directory_uri") . "/js/";
        ...
        $this->add_js("jquery");
        $this->options = get_option(ready_review_config("menu_slug"));
    ...
    public function add_input_text($tabid, $name, $description, $id, $class = '') {
        $html = '
				...
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p> <input class="ready_review_input " placeholder="' . $name . '" name="' . ready_review_config("menu_slug") . '[' . $id . ']" type="text" value="' . $this->options[$id] . '"></div>';
    ...
    public function add_input_text_disabled($tabid, $name, $description, $id, $class = '') {
        $html = '
				...
				<div class="explain">' . $name . '</div><p class="field_description ">' . $description . '</p> <input disabled class="ready_review_input " placeholder="' . $name . '" name="' . ready_review_config("menu_slug") . '[' . $id . ']" type="text" value="' . $this->options[$id] . '"></div>';
        $this->tabs[$tabid]["elements"][] = array("type" => "input_text",
                "html" => $html
        );
    }
    ...
    public function add_input_number($tabid, $name, $description, $id, $min = false, $max = false, $step = false, $class = "") {
        $html = '
				<div class="controls ' . $class . '">
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p>  <input placeholder="' . $name . '" type="number"  class="ready_review_input" value="' . $this->options[$id] . '" name="' . ready_review_config("menu_slug") . '[' . $id . ']"
					' . ($min === false ? '' : ' min = "' . $min . '" ') . '
					' . ($max === false ? '' : ' max = "' . $max . '" ') . '
    ...
    public function add_textarea($tabid, $name, $description, $id, $class = '') {
        $html = '
				<div class="controls ' . $class . '">
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p> <textarea class="ready_review_textarea " placeholder="' . $name . '" name="' . ready_review_config("menu_slug") . '[' . $id . ']"    >' . $this->options[$id] . '</textarea></div>';
    ...
    public function add_textarea_disabled($tabid, $name, $description, $id, $class = '') {
        $html = '
				<div class="controls ' . $class . ' field_disabled">
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p> <textarea disabled class="ready_review_textarea " placeholder="' . $name . '" name="' . ready_review_config("menu_slug") . '[' . $id . ']"    >' . $this->options[$id] . '</textarea></div>';
        $this->tabs[$tabid]["elements"][] = array("type" => "textarea",
                "html" => $html
        );
    }
    ...
    public function add_editor($tabid, $name, $description, $id, $class = '') {
        ob_start();
        wp_editor($this->options[$id], ready_review_config("menu_slug") . '[' . $id . ']');
    ...
    public function add_color($tabid, $name, $description, $id, $class = '') {
        $html = '
				...
				<input type="hidden" id="' . $id . '_color" name="' . ready_review_config("menu_slug") . '[' . $id . ']" value="' . $this->options[$id] . '"/> </br>
    ...
    public function add_image($tabid, $name, $description, $id, $class = '') {
        $html = '
				<div class="controls ' . $class . '">
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p>
				<input type="hidden" id="' . $id . '" name="' . ready_review_config("menu_slug") . '[' . $id . ']" value="' . $this->options[$id] . '"/>
    ...
    public function add_typography($tabid, $name, $description, $id, $class = '') {
        ...
        $html = '
				<div class="controls ' . $class . '">
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p> <div class="ready_review_typo">

					 <input type="hidden" id="' . $id . '_color" name="' . ready_review_config("menu_slug") . '[' . $id . '][color]" value="' . $this->options[$id]['color'] . '"/>
				<input type="text" name=""	class="subo-color-picker" id="' . $id . '_color_selector" value="' . $this->options[$id]['color'] . '" />

						<select class="ready_review_select ready_review_tipsy" original-title="Font family" name="' . ready_review_config("menu_slug") . '[' . $id . '][font]" > ';
        foreach ($fonts as $k => $v) {
            $html .= "<option value='" . $k . "' " . ($this->options[$id]['font'] == $k ? 'selected' : '') . ">" . $v . "</option>";
        }
        $html .= '</select>
						<select class="ready_review_select ready_review_tipsy" original-title="Font style"  name="' . ready_review_config("menu_slug") . '[' . $id . '][style]" > ';
        foreach ($style as $k => $v) {
            $html .= "<option value='" . $k . "' " . ($this->options[$id]['style'] == $k ? 'selected' : '') . ">" . $v . "</option>";
        }
        $html .= '</select>
						<select class="ready_review_select ready_review_tipsy" original-title="Font size" " name="' . ready_review_config("menu_slug") . '[' . $id . '][size]" > ';
        foreach ($sizes as $v) {
    ...
    public function get_font_styles() {
        return array(
                'normal' => __('Normal', 'ready_review'),
                'italic' => __('Italic', 'ready_review'),
                'bold' => __('Bold', 'ready_review'),
                'bold italic' => __('Bold Italic', 'ready_review')
        );
    }
    ...
    public function add_background($tabid, $name, $description, $id, $class = '') {
        ...
        $html = '
				<div class="controls ' . $class . '">
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p><div class="ready_review_background">
					<div class="ready_review_bgstyle">
						<div class="ready_review_bgimage">
						<input type="hidden" id="' . $id . '" name="' . ready_review_config("menu_slug") . '[' . $id . '][bgimage]" value="' . $this->options[$id]['bgimage'] . '"/>
				<img src="' . $this->options[$id]['bgimage'] . '" id="' . $id . '_image" class="image-preview-input"/><br/>
				<a id="' . $id . '_button" class="selector-image button"  >Select Image</a>

				<a id="' . $id . '_buttonclear" class="clear-image button"  >Clear image</a>
						</div>
						<div class="ready_review_bgcolor">
						<input type="hidden" id="' . $id . '_color" name="' . ready_review_config("menu_slug") . '[' . $id . '][bgcolor]" value="' . $this->options[$id]['bgcolor'] . '"/> <br/>
				<input type="text" name=""	class="subo-color-picker" id="' . $id . '_color_selector" value="' . $this->options[$id]['bgcolor'] . '" />
						</div><div class="clear"></div>
					</div>
					<div class="ready_review_bgformat">

						<select class="ready_review_select ready_review_tipsy" original-title="Background repeat"  name="' . ready_review_config("menu_slug") . '[' . $id . '][bgrepeat]" > ';
        foreach ($repeats as $k => $v) {
            $html .= "<option value='" . $k . "' " . ($this->options[$id]['bgrepeat'] == $k ? 'selected' : '') . ">" . $v . "</option>";
        }
        $html .= '</select>
						<select class="ready_review_select ready_review_tipsy" original-title="Background position"  name="' . ready_review_config("menu_slug") . '[' . $id . '][bgposition]" > ';
        foreach ($positions as $k => $v) {
            $html .= "<option value='" . $k . "' " . ($this->options[$id]['bgposition'] == $k ? 'selected' : '') . ">" . $v . "</option>";
        }
        $html .= '</select>
						<select class="ready_review_select ready_review_tipsy" original-title="Background attachament"  name="' . ready_review_config("menu_slug") . '[' . $id . '][bgattachment]" > ';
    ...
    public function get_bg_repeat() {
        return array(
                'no-repeat' => __('No Repeat', 'ready_review'),
                'repeat-x' => __('Repeat Horizontally', 'ready_review'),
                'repeat-y' => __('Repeat Vertically', 'ready_review'),
                'repeat' => __('Repeat All', 'ready_review'),
        );
    }

    public function get_bg_position() {
        return array(
                'top left' => __('Top Left', 'ready_review'),
                'top center' => __('Top Center', 'ready_review'),
                'top right' => __('Top Right', 'ready_review'),
                'center left' => __('Middle Left', 'ready_review'),
                'center center' => __('Middle Center', 'ready_review'),
                'center right' => __('Middle Right', 'ready_review'),
                'bottom left' => __('Bottom Left', 'ready_review'),
                'bottom center' => __('Bottom Center', 'ready_review'),
                'bottom right' => __('Bottom Right', 'ready_review')
        );
    }
    ...
    public function add_select($tabid, $name, $description, $id, $options, $class = "") {
        $html = '
				<div class="controls ' . $class . '">
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p>   <select class=" ready_review_select" name="' . ready_review_config("menu_slug") . '[' . $id . ']" > ';
    ...
    public function add_radio($tabid, $name, $description, $id, $options, $class = "") {
        $html = '
				...
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p>  ';
        foreach ($options as $k => $v) {
            $html .= "<label class='ready_review_label'><input class='ready_review_radio' type=\"radio\" name='" . ready_review_config("menu_slug") . "[" . $id . "]' value='" . $k . "' " . ($this->options[$id] == $k ? 'checked' : '') . ">" . $v . "</label>";
        }
        $html .= '</div>';
    ...
    public function add_multiselect($tabid, $name, $description, $id, $options, $class = "") {
        $html = '
				<div class="controls ' . $class . '">
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p>   <select   name="' . ready_review_config("menu_slug") . '[' . $id . '][]" class="ready_review_multiselect" multiple > ';
        foreach ($options as $k => $v) {
    ...
    public function add_checkbox($tabid, $name, $description, $id, $options, $class = "") {
        $html = '
				...
				<div class="explain">' . $name . '</div><p class="field_description">' . $description . '</p>  ';
        foreach ($options as $k => $v) {
            $html .= "<label class='ready_review_label'><input class='ready_review_checkbox' type=\"checkbox\" name='" . ready_review_config("menu_slug") . "[" . $id . "][]' value='" . $k . "' " . (in_array($k, $this->options[$id]) ? 'checked' : '') . " >" . $v . "</label>";
        }
        $html .= '</div>';
