...
class UPME {
    ...
    function display($args=array()) {
        ...
        if (!$group) {
            ...
            if ($id == null && !is_user_logged_in()) {
                ...
                return $this->view_profile($id, $width, $view, $group, $show_stats, $show_social_bar, $use_in_sidebar, $users_per_page, null, $role, $recent_posts,$logout_redirect,$new_window,$modal,$modal_view,$show_result_count);
            }
        }
        ...
        if ($group) {
            ...
            if (!is_user_logged_in() && !$this->guests_can_view()) {
                ...
                return $this->view_profile($id, $width, $view, $group, $show_stats, $show_social_bar, $use_in_sidebar, $users_per_page, $hide_until_search, $role, $recent_posts,$logout_redirect,$new_window,$modal,$modal_view,$show_result_count);
            }
        }
    }
    ...
    function search_result($args) {
        ...
        if($this->profile_orderby_custom_status || 'yes' == $this->profile_orderby_custom_status){
            ...
            $orderby_meta_where = ' (mta.meta_key = "'.$this->profile_order_field.'" )  AND ';
            $orderby_meta_default_where = ' WHERE (mta.meta_key = "'.$this->profile_order_field.'" )  ' ;
        }
        // End
        ...
        if(!current_user_can('manage_options') && $current_option['profile_view_status']){
        ...
        $this->search_query_search_param = apply_filters('upme_search_user_status',$search_user_status_filters);
        ...
        if (count($this->search_query_search_param) > 0 && isset($this->search_args)) {
            // Results after searching - POST variables are available
            
            $this->search_query_string.=' INNER JOIN ' . $wpdb->usermeta . ' as mt ON (users.ID = mt.user_id) '.$orderby_meta_join.' ' . $post_count_sort . ' WHERE 1=1 AND '. $orderby_meta_where . implode(' ' . $this->search_args['operator'] . ' ', $this->search_query_search_param);

            $this->count_search_query_string.=' INNER JOIN ' . $wpdb->usermeta . ' as mt ON (users.ID = mt.user_id) '.$orderby_meta_join.' ' . $post_count_sort . ' WHERE 1=1 AND '. $orderby_meta_where . implode(' ' . $this->search_args['operator'] . ' ', $this->search_query_search_param);
            ...
            if ($this->upme_args['group_meta'] != '' && $this->upme_args['group_meta_value'] != '') {
                // Search users with provided cusotm field and provided custom value
                $this->search_query_string.=" INNER JOIN " . $wpdb->usermeta . " as mt ON (users.ID = mt.user_id) ".$orderby_meta_join." ". $post_count_sort ." WHERE 1=1 AND ". $orderby_meta_where ." (mt.meta_key = '_upme_search_cache' AND mt.meta_value LIKE '%" . $this->upme_args['group_meta'] . "::" . $this->upme_args['group_meta_value'] . "%')";

                $this->count_search_query_string.=" INNER JOIN " . $wpdb->usermeta . " as mt ON (users.ID = mt.user_id) ".$orderby_meta_join. " " . $post_count_sort ." WHERE 1=1 AND" . $orderby_meta_where . " (mt.meta_key = '_upme_search_cache' AND mt.meta_value LIKE '%" . $this->upme_args['group_meta'] . "::" . $this->upme_args['group_meta_value'] . "%')";
            } else {
                if ($this->search_query_role_param != '') {
                    $this->search_query_string.=" INNER JOIN " . $wpdb->usermeta . " as mt ON (users.ID = mt.user_id) ".$orderby_meta_join. " ";
                    $this->count_search_query_string.=" INNER JOIN " . $wpdb->usermeta . " as mt ON (users.ID = mt.user_id) ".$orderby_meta_join. " ";
                }

                $this->search_query_string.= $post_count_sort.$orderby_meta_join. $orderby_meta_default_where;
                $this->count_search_query_string.= $post_count_sort.$orderby_meta_join. $orderby_meta_default_where;
            }
        }
    ...
    function view_profile($id=null, $width=null, $view=null, $group=null, $show_stats=null, $show_social_bar=null, $use_in_sidebar=null, $users_per_page=null, $hide_until_search=null, $role=null, $recent_posts=null,$logout_redirect=null,$new_window=null,$modal=null,$modal_view=null,$show_result_count=null) {
    ...
    function edit_profile_fields($id, $width=null, $sidebar_class=null) {
        ...
        if ($this->can_edit_profile($this->logged_in_user, $id)) {
                ...
                if ($type == 'usermeta' && $deleted == 0 && $display_field == 1) {
                    ...
                    if ($display_show_status) {
                        ...
                        if (isset($array[$key]['name']) && $name) {
                            ...
                            $display .= '<span>' . apply_filters('upme_edit_profile_label_' . $meta, __($name,'upme')) . '</span></label>';
                        } else {
                            if (isset($array[$key]['icon']) && $icon) {
            ...
            $display .= apply_filters('upme_profile_edit_form_extra_fields','');
            // End Filter
    ...
    function show_profile_fields($id, $view) {
        ...
        if ($upme_settings['display_profile_status'] || ('true' == $this->profile_show_status || 'yes' == $this->profile_show_status) ) {
            ...
            $display.='<div class="upme-field-type"><i class="upme-icon-user"></i><span>' . $profile_status_label . '</span></div>';
        ...
        if (get_option('users_can_register') == '1') {
            ...
            if (!isset($display_errors['status']) || (isset($display_errors['status']) && ("error" == $display_errors['status']))) {
                ...
                if (isset($_POST['upme-register']) && $_POST['display_name'] != '') {
                    ...
                    $display .= apply_filters('upme_profile_head_default_message',$display_msg);
                }
    ...
    function show_register_form($sidebar_class=null, $redirect_to=null) {
        ...
        if (!isset($upme_register->registered) || $upme_register->registered != 1) {
            ...
            if((boolean) $current_option['accepting_terms_and_conditions']){
                $terms_field_title = apply_filters('upme_terms_field_title',__('Please Accpet Terms and Conditions','upme'));
                $display .= '<div class="upme-field upme-edit upme-edit-show">
    ...
    function login($args=array()) {
        ...
        $display .='<div class="upme-field-name upme-field-name-wide login-heading" id="login-heading-' . $this->login_code_count . '">' . __('Login', 'upme') . '</div>';
        $display .='</div>';
    ...
    function show_login_form($register_link_status, $register_text, $custom_register_url, $forgot_link_status, $forgot_text, $custom_forgot_url, $sidebar_class=null, $redirect_to=null) {
        ...
        $display .= apply_filters('upme_forgot_form_extra_fields','');
        // End Filter
        ...
        $display .= apply_filters('upme_login_form_extra_fields','');
        // End Filter
