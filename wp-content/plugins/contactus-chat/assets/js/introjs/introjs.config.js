...
cUsLC_myjq(document).ready(function($) {
    ...
    $('.introjs-skipbutton').on('click',function(){
        ...
        bootbox.confirm("User guide will appear again using the help button above.", function(result) {
            if(result){
                $.ajax({ type: "POST", url: ajax_object.ajax_url, data: {action:'cUsLC_disable_introjs'},
    ...
    intro.setOptions({
        steps: [
            ...
            },
            {
                //2
                element: '#cu_nav_ai',
                intro: "<div class='intro_box_ai intro'><h1>Chat Agent Interface</h1><ul><li>Open Chat Agent Interface in a new window. Full view improves usability when manning chat for long periods of time.</li></ul></div>"
            },
            {
                //2
