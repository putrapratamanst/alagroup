    ...
    <div class="col-md-4">
        <div class="btn-group pull-right">
            ...
            <ul class="dropdown-menu" role="menu">
                <li><a href="<?php echo cUsLC_PARTNER_URL; ?>/index.php?loginName=<?php echo $cUs_API_Account; ?>&userPsswd=<?php echo urlencode($cUs_API_Key); ?>&confirmed=1&redir_url=<?php echo urlencode(trim($createform)); ?>chat" title="Opens ContactUs admin panel in a new window." class="tooltips" data-placement="right" target="_blank">Add a Live Chat Form</a></li>
            </ul>
        </div>
    </div>
</div>
...
<?php
//if (!function_exists('cUsLC_renderFormType')) {
    /*
    ...
    //function cUsLC_renderFormType($form_Type, $cUsLC_API_getFormKeys, $createform) {
...
//        $cUs_API_Key = $aryUserCredentials['API_Key'];
        ...
        if ($cUsLC_API_getFormKeys) {

            $cUs_json = json_decode($cUsLC_API_getFormKeys);
            switch ($cUs_json->status) {
                case 'success':
                    //$aryMyForms = get_option('cUsLC_settings_FORMS'); //get all forms in WP
                    $formOptions        = get_option('cUsLC_FORM_settings');//GET THE NEW FORM OPTIONS
                    $cus_version        = $formOptions['cus_version'];
                    //print_r($formOptions);
                    // = array()

                    //echo count($cUs_json->data);
                    ...
                    <div class="user_form_templates">
                        ...
                        foreach ($cUs_json->data as $oForms => $oForm) {
                            ...
                            if ( cUsLC_allowedFormType($oForm->form_type) ) {
                                //var_dump($oForm);
                                $formID = $oForms;

                                //$default_form_key = get_option('cUsLC_settings_form_key_def');
                                //update_option('cUsLC_settings_form_key', $default_form_key);

                                $default_form_key = get_option('cUsLC_settings_form_key');
                                ...
                                if( !empty($default_form_key) && !isset($formSettings['updated']) && $cus_version === 'tab' ){
                                    ...
                                    update_option('cUsLC_settings_form_'.$oForm->form_id, $formSettings);
                                    $formSettings =  get_option('cUsLC_settings_form_'.$oForm->form_id);

                                    update_option('cUsLC_settings_form_key_def', $default_form_key);
                                    //delete_option('cUsLC_settings_form_key');
                                    //delete_option('cUsLC_settings_FORM');

                                }

                                //delete_option('cUsLC_settings_form_'.$oForm->form_id);
                                ...
                                <form name="cus_form_<?php echo $oForm->form_id; ?>" id="cus_form_<?php echo $oForm->form_id; ?>">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12">
                                                <div class="white-menu">
                                                    ...
                                                    $ablink = $cUsLC_api->parse_deeplink($default_deep_link);
                                                    $ablink = $ablink . '?pageID=90&do=view&formID=' . $oForm->form_id;
                                                    ?>
                                                    <ul class="nav nav-pills">
                                                        <li class="active"><a href="<?php echo cUsLC_CRED_URL; ?>&confirmed=1&redir_url=<?php echo urlencode(trim(cUsLC_PARTNER_URL . '/'.$partnerID . '/en/forms/gt/'.$oForm->form_id.'/formSettings/generalSettings')); ?>" target="_blank" title="Opens ContactUs admin panel in a new window." class="tooltips">General Settings</a></li>
                                                        <li class="dropdown">
                                                            ...
                                                            <ul class="dropdown-menu" role="menu" title="Opens ContactUs admin panel in a new window." class="tooltips"> 
                                                                <li><a href="<?php echo cUsLC_CRED_URL; ?>&confirmed=1&redir_url=<?php echo urlencode(trim($default_deep_link)); ?>%26expand=4" target="_blank" title="Opens ContactUs admin panel in a new window" class="tooltips" data-placement="right">Edit Form Design</a></li>
                                                                <li class="divider"></li>
                                                                <li><a href="<?php echo cUsLC_CRED_URL; ?>&confirmed=1&redir_url=<?php echo urlencode(trim($default_deep_link)); ?>%26expand=5" target="_blank" title="Opens ContactUs admin panel in a new window" class="tooltips" data-placement="right">Edit Tab Design</a></li>
                                                            </ul>
                                                        </li>
                                                        <?php if ($oForm->form_type == 'contact_us') { ?><li class="active"><a href="<?php echo cUsLC_CRED_URL; ?>&confirmed=1&redir_url=<?php echo urlencode(trim($default_deep_link)); ?>%26expand=14%26newTemplate=genericTemplate1" target="_blank" title="Opens ContactUs admin panel in a new window" class="tooltips">Fields</a></li><?php } ?>
                                                        <li class="active"><a href="<?php echo cUsLC_CRED_URL; ?>&confirmed=1&redir_url=<?php echo urlencode(cUsLC_PARTNER_URL . '/'.$partnerID . '/en/forms/gt/'.$oForm->form_id.'/formTriggers/'); ?>" target="_blank" title="Opens ContactUs admin panel in a new window." class="tooltips">Form Triggers</a></li>
                                                        <li class="active"><a href="<?php echo cUsLC_CRED_URL; ?>&confirmed=1&redir_url=<?php echo urlencode(trim($default_deep_link)); ?>%26expand=103" target="_blank" title="Opens ContactUs admin panel in a new window." class="tooltips">Integrations</a></li>
                                                        <li class="active"><a href="<?php echo cUsLC_CRED_URL; ?>&confirmed=1&redir_url=<?php echo urlencode(cUsLC_PARTNER_URL . '/'.$partnerID . '/en/forms/gt/'.$oForm->form_id.'/formSubmissionSettings/formNotifications/'); ?>" target="_blank" title="Opens ContactUs admin panel in a new window." class="tooltips">Notifications</a></li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        <div class="scode-box">
                                                            ...
                                                            <p>
                                                            <h5> Tab Form</h5>
                                                            [show-live-chat-form formkey=”<?php echo $oForm->form_key; ?>" version=”tab"]
                                                            <h5>Inline Form</h5>
                                                            [show-live-chat-form formkey=”<?php echo $oForm->form_key; ?>" version="inline"]
                                                            </p>
                                                        </div>
                                                    </div>
                                    ...
                                    <input type="hidden" name="action" value="cUsLC_setDefaulFormKeyByID">
                                ...
                                //echo $userJScode = '<script type="text/javascript" src="' . cUsLC_ENV_URL . $oForm->form_key . '/contactus.js"></script>';
            ...
            if ($nCF <= 1) {
                ?>
                <a href="<?php echo cUsLC_PARTNER_URL; ?>/index.php?loginName=<?php echo $cUs_API_Account; ?>&userPsswd=<?php echo urlencode($cUs_API_Key); ?>&confirmed=1&redir_url=<?php echo urlencode(trim($createform)) . $form_Type; ?>" target="_blank" class="deep_link_action">Add New Form <span>+</span></a>
                <?php
            }
        }
    //}
...
<script>
    //PLUGIN cUsLC_myjq ENVIROMENT (cUsLC_myjq)
    ...
    cUsLC_myjq(document).ready(function($) {
        ...
        try {
            $(".preview-form").on('click', function(){
                ...
                //console.log(formKey);
                var form_Preview = '<h2>Form Preview</h2><br/><iframe id="formPreview" width="100%" style="min-height:540px;max-height:800px;" src="<?php echo cUsLC_ENV_URL;?>/'+ formKey+'/">';
