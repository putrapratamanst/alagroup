 ...
 * @api
 * @version 0.4
 * @since 2.0 First time this was introduced into ContactUs Chat plugin.
...
class cUsComAPI_LC {
     ...
     * @api
     */
    var $enviroment = cUsLC_API_ENV;
    var $wpApiAccountKey = cUsLC_API_ACC;
    var $wpApiKey = cUsLC_API_AKY;
    ...
    public function getAPICredentials($cUs_email, $cUs_pass) {
        ...
        if( $this->_isCurl() ){
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: WPLC|2.0|' . $this->getIP()));
    ...
    public function createCustomer($postData, $credential) {
        ...
        if (!strlen($postData['fname'])) {
            ...
            if( $this->_isCurl() ){
                ...
                $strCURLOPT .= '&Promotion_Code=WPLC';
                $strCURLOPT .= '&Version=WPLC|2.0';
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                    ...
                    'X-ContactUs-Signature: WPLC|2.0|' . $this->getIP()
                ));
    ...
    public function getDefaultDeepLink($cUsLC_API_getFormKeys) {

        if (empty($cUsLC_API_getFormKeys))
            return FALSE;

        //$cUsMC_API_getKeysResult = $this->getFormKeysData($cUs_API_Account, $cUs_API_Key);
        $cUs_jsonKeys = json_decode($cUsLC_API_getFormKeys);
    ...
    public function resetData() {
        ...
        //PLUGIN OPTIONS
        delete_option('cUsLC_settings');
        delete_option('cUsLC_settings_userData');
        delete_option('cUsLC_settings_FORM');
        delete_option('cUsLC_HOME_settings');
        delete_option('cUsLC_settings_step1');
        delete_option('cUsLC_settings_form_key');
        delete_option('cUsLC_settings_inlinepages');
        delete_option('cUsLC_settings_tabpages');
        delete_option('cUsLC_settings_userCredentials');
        delete_option('cUsLC_settings_default_deep_link_view');
        delete_option('cUsLC_settings_form_id');
        delete_option('cUsLC_settings_intro_hints');

        cUsLC_page_settings_cleaner(); //REMOVE CUSTOM SETTINGS BY PAGE
        cUsLC_inline_shortcode_cleaner(); //REMOVE SHORTCODES
