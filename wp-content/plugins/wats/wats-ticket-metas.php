...
function wats_ticket_details_meta_box($post,$view=0)
{
	...
	foreach ($wats_default_keys_array as $key => $table)
	{
		if ($wats_settings['ticket_'.$table['key'].'_key_enabled'] == 1)
		{
			...
			{
				if ($view == 1)
				$output .= '<div class="wats_select_ticket_'.$table['key'].'_frontend">';
				
				if ($view == 0 && !is_admin())
					$output .= '<div class="wats_select_ticket_'.$table['key'].'_frontend_update_form">';
				
				if (is_admin())

					$output.= '<br />';
				$output .= '<label class="wats_label">'.__($table['name'],'WATS').' : </label>';
				$output .= '<select name="wats_select_ticket_'.$table['key'].'" id="wats_select_ticket_'.$table['key'].'" class="wats_select">';
				
				if (is_array($wats_key_list_of_values))
				foreach ($wats_key_list_of_values as $sub_key => $value)
				{

					$output .= '<option value='.$sub_key;
			...
			{
				$output .= '<br /><label class="wats_label">'.__($table['name'],'WATS').' : </label>';
				foreach ($wats_key_list_of_values as $sub_key => $value)
				{
					if ($sub_key == $key_value || (!$key_value && $sub_key == $wats_settings['default_ticket_'.$table['key']]))
