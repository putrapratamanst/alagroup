...
function wats_load_settings()
{
    ...
    if (!get_option('wats'))
	{
		...
		$default['fsf_unauthenticated_name_mandatory'] = 1;
		$default['fsf_unauthenticated_email_mandatory'] = 1;
		$default['fsf_unauthenticated_website_mandatory'] = 0;
		$default['fsf_unauthenticated_description_mandatory'] = 1;
		$default['fsf_unauthenticated_title_mandatory'] = 1;
		$default['fsf_enable_tinymce'] = 0;
		$default['call_center_ticket_update'] = 0;
		$default['ftuf_enable_tinymce'] = 0;
		$default['wats_ticket_listing_queries'] = array();
		$default['display_list_description_separate_column'] = 0;
		$default['wats_ticket_default_keys'] = array(0 => array('name' => __('Type key','WATS'),
															    'fsf' => 4,
																'atef' => 4,
																'ftdt' => 1,
																'ftuf' => 4,
																...
																'ftltc' => 1,
																'ftlbm' => 5),
													 1 => array('name' => __('Priority key','WATS'),
															    'fsf' => 4,
																'atef' => 4,
																'ftdt' => 1,
																'ftuf' => 4,
																...
																'ftltc' => 1,
																'ftlbm' => 5),
													2 => array('name' => __('Status key','WATS'),
															    'fsf' => 4,
																'atef' => 4,
																'ftdt' => 1,
																'ftuf' => 4,
																...
																'ftltc' => 1,
																'ftlbm' => 5),
													3 => array('name' => __('Product key','WATS'),
															    'fsf' => 4,
																'atef' => 4,
																'ftdt' => 1,
																'ftuf' => 4,
	...
	if ($wats_settings['wats_version'] != $wats_version)
	{
		...
		if (!isset($wats_settings['fsf_unauthenticated_name_mandatory']))
		{
			$wats_settings['fsf_unauthenticated_name_mandatory'] = 1;
		}
		
		if (!isset($wats_settings['fsf_unauthenticated_email_mandatory']))
		{
			$wats_settings['fsf_unauthenticated_email_mandatory'] = 1;
		}
		
		if (!isset($wats_settings['fsf_unauthenticated_website_mandatory']))
		{
			$wats_settings['fsf_unauthenticated_website_mandatory'] = 0;
		}
		
		if (!isset($wats_settings['fsf_unauthenticated_title_mandatory']))
		{
			$wats_settings['fsf_unauthenticated_title_mandatory'] = 1;
		}
		
		if (!isset($wats_settings['fsf_unauthenticated_description_mandatory']))
		{
			$wats_settings['fsf_unauthenticated_description_mandatory'] = 1;
		}
		
		if (!isset($wats_settings['fsf_enable_tinymce']))
		{
			$wats_settings['fsf_enable_tinymce'] = 0;
		}
		
		if (!isset($wats_settings['call_center_ticket_update']))
		{
			$wats_settings['call_center_ticket_update'] = 0;
		}
		...
		if (!isset($wats_settings['display_list_description_separate_column']))
		{
			$wats_settings['display_list_description_separate_column'] = 0;
		}
		
		if (!isset($wats_settings['wats_ticket_default_keys']))
		{
			$wats_settings['wats_ticket_default_keys'] = array(0 => array('name' => __('Type key','WATS'),
															    'fsf' => 4,
																'atef' => 4,
																'ftdt' => 1,
																'ftuf' => 4,
		...
		foreach ($wats_settings['wats_ticket_custom_fields'] as $key => $table)
		{
			if (!isset($wats_settings['wats_ticket_custom_fields'][$key]['ftlbm']))
				$wats_settings['wats_ticket_custom_fields'][$key]['ftlbm'] = 5;
		}
		
		foreach ($wats_settings['wats_ticket_default_keys'] as $key => $table)
		{
			if (!isset($wats_settings['wats_ticket_default_keys'][$key]['ftlbm']))
				$wats_settings['wats_ticket_default_keys'][$key]['ftlbm'] = 5;
		}
...
function wats_admin_remove_option_entry()
{
	...
	if ($type == "wats_custom_selector")
	{
		...
		if (isset($wats_ticket_custom_field_values[$idcat]))
		{
			...
			if (isset($table['type']) && ($table['type'] == 1 || $table['type'] == 3))
			{
				$values = $table['values'];
...
function wats_admin_remove_notification_rule_entry()
{
	...
	{
		$message_result = array('id' => $idvalue,'success' => "FALSE", 'error' => __("Error : rule doesn't exist!",'WATS'));
	}
...
function wats_admin_insert_notification_rule_entry()
{
	...
	$idcountry = '';
	if ($wats_settings['profile_country_enabled'] == 1 && isset($_POST['idcountry']))
		$idcountry = stripslashes_deep($_POST['idcountry']);

	$idcompany = '';
	if ($wats_settings['profile_company_enabled'] == 1 && isset($_POST['idcompany']))
		$idcompany = stripslashes_deep($_POST['idcompany']);
		
	$idduedatefield = stripslashes_deep($_POST['idduedatefield']);
	$idauthorcheck = stripslashes_deep($_POST['idauthorcheck']);
	$idownercheck = stripslashes_deep($_POST['idownercheck']);
	$idadminscheck = stripslashes_deep($_POST['idadminscheck']);
	$idupdaterscheck = stripslashes_deep($_POST['idupdaterscheck']);
	$due_date_notification_interval = stripslashes_deep($_POST['due_date_notification_interval']);
	$due_date_notification_start = stripslashes_deep($_POST['due_date_notification_start']);
	...
	if (strlen($_POST['listvalue']) == 0 && $idauthorcheck == 'false' && $idownercheck == 'false' && $idadminscheck == 'false' && $idupdaterscheck == 'false')
	{
		$message_result = array('id' => "", 'idvalue' => "",'success' => "FALSE", 'error' => __("Error : please create an entry for the distribution list or select someone to notify!",'WATS'));
	}
	else if ($idrulescope == 3 && (!wats_is_numeric($due_date_notification_interval) || !wats_is_numeric($due_date_notification_start)))
	{
		$message_result = array('id' => "", 'idvalue' => "",'success' => "FALSE", 'error' => __("Error : please enter a number for the due date notifcation start and interval!",'WATS'));
	}
	else
    {
		$wats_notification_rules = get_option('wats_notification_rules');
		...
		if ($wats_settings['profile_country_enabled'] == 1 && strlen($idcountry))
			$rule .= "country:".$idcountry.";";
		if ($wats_settings['profile_company_enabled'] == 1 && strlen($idcompany))
			$rule .= "company:".$idcompany.";";
		$rule .= "category:".$idcategorie.";";
		
		if ($idrulescope == 3)
		{
			$rule .= "duedatefield:".$idduedatefield.";";
			$rule .= "duedatestart:".$due_date_notification_start.";";
			$rule .= "duedateinterval:".$due_date_notification_interval.";";
		}
			
		$rule .= "notify_author:".$idauthorcheck.";";
		$rule .= "notify_owner:".$idownercheck.";";
		$rule .= "notify_admins:".$idadminscheck.";";
		$rule .= "notify_updaters:".$idupdaterscheck.";";
...
/**************************************************************/
/*                                                            */
/* Fonction de suppression d'une query pour le ticket listing */
/*                               			                  */
/**************************************************************/

function wats_admin_remove_ticket_listing_query()
{
	global $wats_settings;
	...
	$queriestoremove = isset($_POST['liste']) ? json_decode(stripslashes($_POST['liste'])) : array();	
	
	if (count($queriestoremove) == 0)
		$message_result = array('success' => "FALSE", 'error' => __('Error','WATS').' : '.__('no query selected!','WATS'));
	else
    {
		$invalid_queries = '';
		$removed_queries = '';
		
		$wats_ticket_listing_queries = $wats_settings['wats_ticket_listing_queries'];
	
		foreach ($queriestoremove as $key => $querytoremove)
		{
			if (isset($wats_ticket_listing_queries[$querytoremove]))
			{
				unset($wats_ticket_listing_queries[$querytoremove]);
				$removed_queries .= (strlen($removed_queries) > 0) ? ' '.$querytoremove : $querytoremove;
			}
			else
				$invalid_queries .= (strlen($invalid_queries) > 0) ? ' '.$querytoremove : $querytoremove;
		}

		if (strlen($removed_queries))
		{
			$wats_settings['wats_ticket_listing_queries'] = array_values($wats_ticket_listing_queries);
			update_option('wats', $wats_settings);
		}
		
		if (strlen($invalid_queries) > 0)
			$invalid_queries = __('Error','WATS').' : '.__('invalid queries','WATS').' ('.$invalid_queries.')!';
		
		if (strlen($removed_queries) == 0)
			$message_result = array('success' => "FALSE", 'error' => $invalid_queries);
		else
		{
			$removed_queries = __('Queries successfully removed','WATS').' ('.$removed_queries.')! ';
			$message_result = array('success' => "TRUE", 'output' => wats_options_display_frontend_queries_table(), 'error' => $removed_queries.$invalid_queries);
		}
  	}
...
/*******************************************************/
/*                                                     */
/* Fonction d'ajout d'une query pour le ticket listing */
/*                               			           */
/*******************************************************/

function wats_admin_insert_ticket_listing_query($idvalue)
{
	global $wats_settings;
	...
	$idpriority = stripslashes_deep($_POST['idpriority']);
	$idstatus = stripslashes_deep($_POST['idstatus']);
	$idstatusoperator = stripslashes_deep($_POST['idstatusoperator']);
	$idproduct = stripslashes_deep($_POST['idproduct']);
	$idauthor = stripslashes_deep($_POST['idauthor']);
	$idowner = stripslashes_deep($_POST['idowner']);
	$queryname = stripslashes_deep($_POST['queryname']);
	...
	if (strlen($queryname) == 0)
		$message_result = array('success' => "FALSE", 'error' => __("Error : please enter a name for the query!",'WATS'));
	else if ($idauthor != 0 && $idauthor != 1)
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for the author!",'WATS'));
	else if ($idowner != 0 && $idowner != 1 && $idowner != 2)
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for the owner!",'WATS'));
	else if (!isset($wats_ticket_product[$idproduct]) && $idproduct != 0)
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for the product!",'WATS'));
	else if (!isset($wats_ticket_priority[$idpriority]) && $idpriority != 0)
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for the priority!",'WATS'));
	else if (!isset($wats_ticket_status[$idstatus]) && $idstatus != 0)
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for the status!",'WATS'));
	else if ($idstatusoperator != 0 && $idstatusoperator != 1)
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for the status operator!",'WATS'));
	else if (!isset($wats_ticket_type[$idtype]) && $idtype != 0)
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for the type!",'WATS'));
	else
	{
		...
		if ($idvalue == -1)
		{
			$wats_ticket_listing_queries[] = array('name' => $queryname,
													   'type' => $idtype,
													   'priority' => $idpriority,
													   'status' => $idstatus,
													   'status_op' => $idstatusoperator,
													   'product' => $idproduct,
													   'author' => $idauthor,
													   'owner' => $idowner);
			$message = __("Entry successfully added!",'WATS');
		}
		else
		{
			$wats_ticket_listing_queries[$idvalue] = array('name' => $queryname,
													   'type' => $idtype,
													   'priority' => $idpriority,
													   'status' => $idstatus,
													   'status_op' => $idstatusoperator,
													   ...
													   'author' => $idauthor,
													   'owner' => $idowner);
			$message = __("Entry successfully updated!",'WATS');
		}
		
		$wats_settings['wats_ticket_listing_queries'] = $wats_ticket_listing_queries;
		update_option('wats', $wats_settings);
		$output = wats_options_display_frontend_queries_table();
		$message_result = array('success' => "TRUE", 'output' => $output, 'error' => $message);
	}
...
function wats_admin_insert_ticket_custom_field($idvalue)
{
	...
	$idftlbm = stripslashes_deep($_POST['idftlbm']);
	$idtype = stripslashes_deep($_POST['idtype']);
	$customfieldname = stripslashes_deep($_POST['customfieldname']);
	$customfieldmetakey = stripslashes_deep($_POST['customfieldmetakey']);
	$idnewvalue = (isset($_POST['idnewvalue']) && wats_is_numeric($_POST['idnewvalue'])) ? intval(stripslashes_deep($_POST['idnewvalue'])) : -1;
	...
	else if ($idftlbm != 0 && $idftlbm != 4 && $idftlbm != 5)
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for frontend ticket listing bulk modification!",'WATS'));
	}
	else if ($idtype != 0 && $idtype != 1 && $idtype != 2 && $idtype != 3)
	{
	...
	else
	{
		...
		if (is_array($wats_ticket_custom_field_values))
		{
			...
			$wats_ticket_custom_field_values = array();
		...
		else
		{
			...
			else
			{
				...
				{
					$new_wats_ticket_custom_field_values = array();
					$beginning = array();
					$end = array();
					foreach ($wats_ticket_custom_field_values as $key => $custom_field_entry)
					{
						if ($key < $idnewvalue && $key != $idvalue)
							$beginning[] = $custom_field_entry;
						
						if ($key >= $idnewvalue && $key != $idvalue)
							$end[] = $custom_field_entry;
					}
					$new_wats_ticket_custom_field_values = $beginning;
					$new_wats_ticket_custom_field_values[$idnewvalue] = $table;
					$wats_ticket_custom_field_values = array_values(array_merge($new_wats_ticket_custom_field_values,$end));
				}
...
function wats_admin_remove_ticket_custom_field()
{
	...
	$customfieldstoremove = isset($_POST['liste']) ? json_decode(stripslashes($_POST['liste'])) : array();	
	
	if (count($customfieldstoremove) == 0)
		$message_result = array('success' => "FALSE", 'error' => __('Error','WATS').' : '.__('no custom field selected!','WATS'));
	else
    {
		...
		foreach ($customfieldstoremove as $key => $customfieldtoremove)
		{
			if (isset($wats_ticket_custom_field_values[$customfieldtoremove]))
			{
				unset($wats_ticket_custom_field_values[$customfieldtoremove]);
				$removed_custom_fields .= (strlen($removed_custom_fields) > 0) ? ' '.$customfieldtoremove : $customfieldtoremove;
			}
			else
				$invalid_custom_fields .= (strlen($invalid_custom_fields) > 0) ? ' '.$customfieldtoremove : $customfieldtoremove;
		}

		if (strlen($removed_custom_fields))
		{
			$wats_settings['wats_ticket_custom_fields'] = array_values($wats_ticket_custom_field_values);
			update_option('wats', $wats_settings);
		}
		
		if (strlen($invalid_custom_fields) > 0)
			$invalid_custom_fields = __('Error','WATS').' : '.__('invalid custom fields','WATS').' ('.$invalid_custom_fields.')!';
		
		if (strlen($removed_custom_fields) == 0)
			$message_result = array('success' => "FALSE", 'error' => $invalid_custom_fields);
		else
		{
			$removed_custom_fields = __('Custom fields successfully removed','WATS').' ('.$removed_custom_fields.')! ';
			$message_result = array('success' => "TRUE", 'output' => wats_options_display_custom_fields_table(), 'error' => $removed_custom_fields.$invalid_custom_fields);
		}
  	}
...
function wats_admin_add_notification_rules_interface($resultsup,$resultadd,$idsup,$idadd,$value,$input)
{
	...
	echo '</select>';
	echo '<span id="wats_rules_due_date" style="padding-left:40px; display:none;">';
	echo '<label style="font-size:13px;">'.__('Due date field','WATS').' : </label>';
	...
	foreach ($wats_ticket_custom_field_values as $key => $table)
	{
		...
		{
			$code .= '<option value="'.$table['meta_key'].'">'.$table['name'].'</option>';
		}
	}
	if (strlen($code))
	{
		echo '<select name="notification_rules_select_due_date_field" id="notification_rules_select_due_date_field">'.$code.'</select> | ';
		echo sprintf(__('Notify every %1$s days starting %2$s days before due date.','WATS'),
		' <input type="text" name="rule_due_date_notification_interval" id="rule_due_date_notification_interval" size="5" style="width:30px;" class="regular-text" /> ',
		' <input type="text" name="rule_due_date_notification_start" id="rule_due_date_notification_start" size="5" style="width:30px;" class="regular-text" /> ');
	}
	else
		echo __('Please configure a datepicker text input custom field first!','WATS');
	...
	{
		if (function_exists('wats_build_country_list'))
		{
	...
	echo '<tr><th><label>'.__('Registered users to notify','WATS').'</label></th><td>';
	echo '<input type="checkbox" name="notification_rules_author_check" id="notification_rules_author_check" value="" />'.__('Ticket author','WATS').'<br />';
	echo '<input type="checkbox" name="notification_rules_owner_check" id="notification_rules_owner_check" value="" />'.__('Ticket owner','WATS').'<br />';
	echo '<input type="checkbox" name="notification_rules_all_admins_check" id="notification_rules_all_admins_check" value="" />'.__('All administrators','WATS').'<br />';
	echo '<input type="checkbox" name="notification_rules_all_updaters_check" id="notification_rules_all_updaters_check" value="" />'.__('Ticket updaters','WATS').'<br />';
	echo '</td><td></td></tr>';
...
function wats_admin_display_notification_rules_list()
{
	...
	if (is_array($wats_notification_rules))
	{
		foreach (array_keys($wats_notification_rules) AS $key)
		{
			...
			{
				$x = 1;
				$rules = wats_admin_display_notification_rule(wats_admin_build_notification_rule($rule));
...
/**************************************************************/
/*                                                            */
/* Fonction de mise à jour d'un custom query pour les tickets */
/*                               			                  */
/**************************************************************/

function wats_admin_update_ticket_custom_query()
{
	global $wats_settings;
	...
	$wats_ticket_listing_queries = $wats_settings['wats_ticket_listing_queries'];
	if (!isset($wats_ticket_listing_queries[$idvalue]))
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid query to modify!",'WATS'));
		echo json_encode($message_result);
		exit;
	}
	
	wats_admin_insert_ticket_listing_query($idvalue);
...
/*************************************************************************/
/*                                                                       */
/* Fonction de récupération d'une ligne de la table des queries à éditer */
/*                                                                       */
/*************************************************************************/

function wats_admin_options_get_custom_query_table_row()
{
	global $wats_settings;
	...
	$wats_ticket_priority = isset($wats_settings['wats_priorities']) ? $wats_settings['wats_priorities'] : array();
	$wats_ticket_type = isset($wats_settings['wats_types']) ? $wats_settings['wats_types'] : array();
	$wats_ticket_status = isset($wats_settings['wats_statuses']) ? $wats_settings['wats_statuses'] : array();
	$wats_ticket_product = isset($wats_settings['wats_products']) ? $wats_settings['wats_products'] : array();
	
	$wats_ticket_priority = array(0 => __('All','WATS')) + $wats_ticket_priority;
	$wats_ticket_type = array(0 => __('All','WATS')) + $wats_ticket_type;
	$wats_ticket_status = array(0 => __('All','WATS')) + $wats_ticket_status;
	$wats_ticket_product = array(0 => __('All','WATS')) + $wats_ticket_product;
	...
	{
		$table = $wats_ticket_listing_queries[$idvalue];
		
		$output = '<td>'.$idvalue.'</td><td><input type="text" id="wats_input_ticket_query_name_tl_queries'.$idvalue.'" size=30 value="'.esc_attr($table['name']).'" /></td>';
		$output .= '<td><select name="wats_select_ticket_type_tl_queries'.$idvalue.'" id ="wats_select_ticket_type_tl_queries'.$idvalue.'" size="1">'.wats_fill_drop_down_no_echo($wats_ticket_type,$table['type']).'</select></td>';
		$output .= '<td><select name="wats_select_ticket_priority_tl_queries'.$idvalue.'" id ="wats_select_ticket_priority_tl_queries'.$idvalue.'" size="1">'.wats_fill_drop_down_no_echo($wats_ticket_priority,$table['priority']).'</select></td>';
		$output .= '<td><select name="wats_select_ticket_status_operator_tl_queries'.$idvalue.'" id="wats_select_ticket_status_operator_tl_queries'.$idvalue.'" class="wats_select">'.wats_fill_drop_down_no_echo(array(0 => '==', 1 => '!='),$table['status_op']).'</select> <select name="wats_select_ticket_status_tl_queries'.$idvalue.'" id ="wats_select_ticket_status_tl_queries'.$idvalue.'" size="1">'.wats_fill_drop_down_no_echo($wats_ticket_status,$table['status']).'</select></td>';
		$output .= '<td><select name="wats_select_ticket_product_tl_queries'.$idvalue.'" id ="wats_select_ticket_product_tl_queries'.$idvalue.'" size="1">'.wats_fill_drop_down_no_echo($wats_ticket_product,$table['product']).'</select></td>';
		$output .= '<td><select name="wats_select_ticket_author_tl_queries'.$idvalue.'" id="wats_select_ticket_author_tl_queries'.$idvalue.'" class="wats_select">'. wats_fill_drop_down_no_echo(array(__('Any','WATS'),__('Current user','WATS')),$table['author']).'</select></td>';
		$output .= '<td><select name="wats_select_ticket_owner_tl_queries'.$idvalue.'" id="wats_select_ticket_owner_tl_queries'.$idvalue.'" class="wats_select">'.wats_fill_drop_down_no_echo(array(__('Any','WATS'),__('None','WATS'),__('Current user','WATS')),$table['owner']).'</select></td>';		
		$output .= '<td><input type="submit" name="wats_save_query_'.$idvalue.'" id="wats_save_query_'.$idvalue.'" value="'.__('Save','WATS').'" class="button-primary" /></td>';
		$output .= '<td><input type="checkbox" name="customquerycheck_'.$idvalue.'" id="customquerycheck_'.$idvalue.'" value="'.$idvalue.'" /></td>';
...
/********************************************************************************/
/*                                                                              */
/* Fonction d'affichage de la table des queries pour le frontend ticket listing */
/*                                                                              */
/********************************************************************************/

function wats_options_display_frontend_queries_table()
{
	global $wats_settings;
	
	wats_load_settings();
	
	$wats_ticket_listing_queries = (isset($wats_settings['wats_ticket_listing_queries'])) ? $wats_settings['wats_ticket_listing_queries'] : 0;
	$wats_ticket_priority = isset($wats_settings['wats_priorities']) ? $wats_settings['wats_priorities'] : array();
	$wats_ticket_type = isset($wats_settings['wats_types']) ? $wats_settings['wats_types'] : array();
	$wats_ticket_status = isset($wats_settings['wats_statuses']) ? $wats_settings['wats_statuses'] : array();
	$wats_ticket_product = isset($wats_settings['wats_products']) ? $wats_settings['wats_products'] : array();
	...
	$output = '<table class="widefat" cellspacing="0" id="tableticketfontendqueriestable" style="text-align:center;"><thead><tr class="thead">';
	$output .= '<th scope="col" class="manage-column" width="10%" style="text-align:center;">ID</th>';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Query name','WATS').'</th>';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Type','WATS').'</th>';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Priority','WATS').'</th>';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Status','WATS').'</th>';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Product','WATS').'</th>';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Author','WATS').'</th>';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Owner','WATS').'</th>';
	...
	$author_array = array(__('Any','WATS'),__('Current user','WATS'));
	$owner_array = array(__('Any','WATS'),__('None','WATS'),__('Current user','WATS'));
	$operator = array('==','!=');
	
	if (is_array($wats_ticket_listing_queries))
	foreach ($wats_ticket_listing_queries as $key => $table)
	{
		$x = 1;
		$output .= '<tr><td>'.$key.'</td><td>'.esc_html($table['name']).'</td>';
		$output .= '<td>'.esc_html($wats_ticket_type[$table['type']]).'</td>';
		$output .= '<td>'.esc_html($wats_ticket_priority[$table['priority']]).'</td>';
		$output .= '<td>'.esc_html($operator[$table['status_op']]).' '.esc_html($wats_ticket_status[$table['status']]).'</td>';
		$output .= '<td>'.esc_html($wats_ticket_product[$table['product']]).'</td>';
		$output .= '<td>'.$author_array[$table['author']].'</td>';
		$output .= '<td>'.$owner_array[$table['owner']].'</td>';
		$output .= '<td><input type="submit" name="wats_edit_query_'.$key.'" id="wats_edit_query_'.$key.'" value="'.__('Edit','WATS').'" class="button-primary" /></td>';
		$output .= '<td><input type="checkbox" name="customquerycheck_'.$key.'" id="customquerycheck_'.$key.'" value="'.$key.'" /></td>';
		$output .= '</tr>';
	}
...
function wats_admin_options_get_custom_field_table_row()
{
	...
	{
		$table = $wats_ticket_custom_field_values[$idvalue];
		$output = '<td><input type="text" id="customfieldid'.$idvalue.'" size=30 value="'.esc_attr($idvalue).'" /></td><td><input type="text" id="customfieldsdisplayname'.$idvalue.'" size=30 value="'.esc_attr($table['name']).'" /></td><td><input type="text" id="customfieldsmetakey'.$idvalue.'" size=30 value="'.esc_attr($table['meta_key']).'" /></td>';
		...
		if (isset($table['type']) && $table['type'] == 1)
			$output .= ' selected';
		$output .= '>'.__('Drop down selector','WATS').'</option><option value="2"';
		if (isset($table['type']) && $table['type'] == 2)
			$output .= ' selected';
		$output .= '>'.__('Text input with datepicker','WATS').'</option><option value="3"';
		if (isset($table['type']) && $table['type'] == 3)
			$output .= ' selected';
		$output .= '>'.__('Radio button','WATS').'</option></select></td>';
		...
		$output .= '<td>'.wats_options_display_custom_fields_selectors_field(array(0,4,5),'ftlbm'.$idvalue,$table['ftlbm']).'</td>';
...
/*********************************************************************************/
/*                                                                               */
/* Fonction de récupération d'une ligne de la table des clés par défaut à éditer */
/*                                                                               */
/*********************************************************************************/

function wats_admin_options_get_default_field_table_row()
{
	global $wats_settings;
	...
	$wats_ticket_default_field_values = $wats_settings['wats_ticket_default_keys'];
	if (!isset($wats_ticket_default_field_values[$idvalue]))
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid default key to modify!",'WATS'));
	}
	else
	{
		$table = $wats_ticket_default_field_values[$idvalue];
		$output = '<td>'.esc_html($table['name']).'</td>';
		$output .= '<td>'.wats_options_display_custom_fields_selectors_field(array(0,4,5),'fsf'.$idvalue,$table['fsf']).'</td>';
		$output .= '<td>'.wats_options_display_custom_fields_selectors_field(array(0,1,2,3,4,5),'atef'.$idvalue,$table['atef']).'</td>';
		$output .= '<td>'.wats_options_display_custom_fields_selectors_field(array(0,1,2),'ftdt'.$idvalue,$table['ftdt']).'</td>';
		$output .= '<td>'.wats_options_display_custom_fields_selectors_field(array(0,4,5),'ftuf'.$idvalue,$table['ftuf']).'</td>';
		...
		$output .= '<td>'.wats_options_display_custom_fields_selectors_field(array(0,1,2),'ftltc'.$idvalue,$table['ftltc']).'</td>';
		$output .= '<td>'.wats_options_display_custom_fields_selectors_field(array(0,4,5),'ftlbm'.$idvalue,$table['ftlbm']).'</td>';
		$output .= '<td><input type="submit" name="wats_save_default_key'.$idvalue.'" id="wats_save_default_key'.$idvalue.'" value="'.__('Save','WATS').'" class="button-primary" /></td>';
...
/***********************************************/
/*                                             */
/* Fonction de mise à jour des clés par défaut */
/*                                             */
/***********************************************/

function wats_admin_update_ticket_default_field()
{
	global $wats_settings;
	...
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for frontend submission form default key visibility!",'WATS'));
	}
	else if ($idatef != 0 && $idatef != 1 && $idatef != 2 && $idatef != 3 && $idatef != 4 && $idatef != 5)
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for admin ticket edition page default key visibility!",'WATS'));
	}
	else if ($idftdt != 0 && $idftdt != 1 && $idftdt != 2)
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for frontend ticket display template default key visibility!",'WATS'));
	}
	else if ($idftuf != 0 && $idftuf != 4 && $idftuf != 5)
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for frontend ticket update form default key visibility!",'WATS'));
	}
	else if ($idftlf != 0 && $idftlf != 1 && $idftlf != 2)
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for frontend ticket listing filter default key visibility!",'WATS'));
	}
	else if ($idftltc != 0 && $idftltc != 1 && $idftltc != 2)
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for frontend ticket listing table column default key visibility!",'WATS'));
	}
	else if ($idftlbm != 0 && $idftlbm != 4 && $idftlbm != 5)
	{
		$message_result = array('success' => "FALSE", 'error' => __("Error : please select a valid value for frontend ticket listing bulk modification!",'WATS'));
	...
	else
	{
		...
		$table['ftltc'] = $idftltc;
		$table['ftlbm'] = $idftlbm;
		$wats_ticket_default_field_values[$idvalue] = $table;
		$wats_settings['wats_ticket_default_keys'] = $wats_ticket_default_field_values;
		update_option('wats', $wats_settings);
		$output = wats_options_display_default_keys_table();
		$message_result = array('success' => "TRUE", 'output' => $output, 'error' => __("Default key successfully updated!",'WATS'));
	}
...
/********************************************************/
/*                                                      */
/* Fonction d'affichage de la table des clés par défaut */
/*                                                      */
/********************************************************/

function wats_options_display_default_keys_table()
{
	global $wats_settings, $wats_custom_fields_selectors;

	wats_load_settings();
																
	$wats_ticket_default_keys = (isset($wats_settings['wats_ticket_default_keys'])) ? $wats_settings['wats_ticket_default_keys'] : 0;

	$output = '<table class="widefat" cellspacing="0" id="tabledefaultkey" style="text-align:center;"><thead><tr class="thead">';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Ticket key','WATS').'</th>';
	...
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Frontend ticket listing bulk modification','WATS').'</th>';
	$output .= '<th scope="col" class="manage-column" style="text-align:center;">'.__('Edit','WATS').'</th>';
    $output .= '</tr></thead><tbody class="list:user user-list">';
	$x = 0;
	if (is_array($wats_ticket_default_keys))
	foreach ($wats_ticket_default_keys as $key => $table)
	{
		$x = 1;
		$output .= '<tr><td>'.esc_html($table['name']).'</td>';
		...
		$output .= '<td>'.$wats_custom_fields_selectors[$table['ftlbm']].'</td>';
		$output .= '<td><input type="submit" name="wats_edit_default_key'.$key.'" id="wats_edit_default_key'.$key.'" value="'.__('Edit','WATS').'" class="button-primary" /></td>';
		$output .= '</tr>';
	}
...
function wats_options_display_custom_fields_table()
{
	...
	foreach ($wats_ticket_custom_field_values as $key => $table)
	{
		...
		if (!isset($table['type']) || $table['type'] == 0)
			...
			$output .= '<td>'.__('Text input with datepicker','WATS').'</td>';
		else  if ($table['type'] == 3)
			$output .= '<td>'.__('Radio button','WATS').'</td>';
		$output .= '<td>'.$wats_custom_fields_selectors[$table['fsf']].'</td>';
		$output .= '<td>'.$wats_custom_fields_selectors[$table['atef']].'</td>';
		$output .= '<td>'.$wats_custom_fields_selectors[$table['ftdt']].'</td>';
		$output .= '<td>'.$wats_custom_fields_selectors[$table['ftuf']].'</td>';
		...
		$output .= '<td><input type="checkbox" name="customfieldcheck'.$key.'" id="customfieldcheck'.$key.'" value="'.$key.'" /></td>';
		$output .= '</tr>';
	}
	
	if ($x == 0)
        $output .= '<tr valign="middle"><td colspan="13" style="text-align:center">'.__('No entry','WATS').'</td></tr>';
...
function wats_options_display_custom_fields_interface()
{
	...
	$output .= '<select name="wats_custom_field_type" id ="wats_custom_field_type" size="1"><option value="0">'.__('Text input','WATS').'</option><option value="1">'.__('Drop down selector','WATS').'</option><option value="2">'.__('Text input with datepicker','WATS').'</option><option value="3">'.__('Radio button','WATS').'</option></select></td></tr>';
	...
	$output .= wats_options_display_custom_fields_selectors(array(0,4,5),'ftlbm',__('Frontend ticket listing bulk modification','WATS'));
	$output .= '</table><br />';
...
function wats_options_manage_notification_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("notification_admin_tip");>'.__('Notifications','WATS').' :</a></h3>';
	...
	if ($wats_settings['ticket_update_notification_all_tickets'] == 1)
		echo ' checked';
	echo '> '.__('Notify admins by email when ticket is updated. Applies to all tickets and will notify all admins.','WATS').'</td></tr><tr><td>';
	...
	echo '<tr><td><select name="ticket_notification_custom_list" id="ticket_notification_custom_list">';
	$list = array("Nobody","Any user","Admins only");
	foreach ($list as $key => $value)
	{
		echo '<option value="'.$key.'"';
		if ($key === $wats_settings['ticket_notification_custom_list'])
			echo ' selected';
		echo '>'.$value.'</option>';
	}
	echo '</select> '.__('can define per ticket custom mailing list for update notification.','WATS').'</td></tr><tr><td>';
	...
	echo __('If the option is disabled here then it will be disabled for everybody and it can\'t be enabled individually. ','WATS');
	echo __('The update notification is sent for the following events : new comment added to a ticket, ownership, priority, status or type change in the ticket edition admin page.','WATS').'<br /><br />';
	echo __('These are global options that can be enabled or disabled individually under user profile if the bypass option is set. ','WATS');
	...
	echo __('Warning : with these options enabled the system may send a lot of emails, especially if you have a lot of users. Please make sure that you really understand the implications before enabling these.','WATS').'<br /><br />';
	echo __('The latest option allows users to define a mailing list for each ticket so that specific email addresses can be notified of updates.','WATS');
	echo '</div></td></tr></table><br />';

	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("notification_signature_tip");>'.__('Mail notifications signature','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("wats_source_email_tip");>'.__('Source email address for notifications','WATS').' :</a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<br /><h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("notification_rule_tip");>'.__('Ticket notification rules','WATS').' :</a></h3><br />';
	...
	echo __('Warning : if you enter multiple email addresses, please separate them with a comma ",".','WATS').'<br /><br />';
	echo __("Note : if you select the author, the owner, the admins and the updaters, all those need to be registered users with a valid email address to be notified. Unregistered authors and updaters aren't notified through rules.",'WATS');
	echo '</div>';
...
function wats_options_manage_ticket_submission_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("ticket_edition_media_upload_tip");>'.__('Media upload','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo __('Check this option if you want to allow media upload while creating (frontend and admin sides) and updating tickets (frontend and admin sides). This will allow users to attach media files to tickets.','WATS').'</div></td></tr></table><br />';
	
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("ticket_edition_media_upload_tabs_tip");>'.__('Media upload tabs','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	echo '<tr><td><input type="checkbox" name="ticket_edition_media_upload_tabs"';
	if ($wats_settings['ticket_edition_media_upload_tabs'] == 1)
		echo ' checked';
	echo '> '.__('Allow full media library browsing during media upload','WATS').'</td></tr><tr><td>';
	echo '<div class="wats_tip" id="ticket_edition_media_upload_tabs_tip">';
	echo __('Check this option if you want to allow media library browsing during media upload while creating and editing tickets. This will allow users to view all the files in the library. If the option is disabled users have only access to their own files','WATS').'.</div></td></tr></table><br />';

	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("guestlist_tip");>'.__('Shared guest user','WATS').' : </a></h3>';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("frontendsubmitformaccess_tip");>'.__('Frontend submission form access','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '>'.__('Enable frontend ticket submission form for any visitor','WATS').'</td></tr>';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("frontendsubformstatus_tip");>'.__('Frontend submission form ticket status','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo __('Set this option to define the ticket publication status upon ticket submission. It is advisable to set unauthenticated users tickets status to \'pending\' to allow admin moderation before publication and limit SPAM.','WATS');
	echo '</div></td></tr></table><br />';
	
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("enabletinymce_tip");>'.__('Frontend forms text editor','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	echo '<tr><td><input type="checkbox" name="fsf_enable_tinymce" ';
	echo ($wats_settings['fsf_enable_tinymce'] == 1) ? 'checked' : '';
	echo '> '.__('Enable the HTML text editor for ticket description','WATS').' </td></tr>';
	echo '<tr><td><input type="checkbox" name="ftuf_enable_tinymce" ';
	echo ($wats_settings['ftuf_enable_tinymce'] == 1) ? 'checked' : '';
	echo '> '.__('Enable the HTML text editor for ticket update form','WATS').' </td></tr>';
	echo '<tr><td>';
	wats_options_premium_only();
	echo '<div class="wats_tip" id="enabletinymce_tip">';
	echo __('Select the first option to enable the HTML editor for the ticket description in the frontend submission form and the second one to enable the HTML editor for the ticket update form.','WATS');
	echo '</div></td></tr></table><br />';
	
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("mandatoryfieldfrontendsub_tip");>'.__('Mandatory fields for frontend ticket submission','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	echo '<tr><td><input type="checkbox" name="fsf_unauthenticated_name_mandatory" ';
	echo ($wats_settings['fsf_unauthenticated_name_mandatory'] == 1) ? 'checked' : '';
	echo '> '.__('Name','WATS').' '.__('(unauthenticated user)','WATS').' </td></tr>';
	echo '<tr><td><input type="checkbox" name="fsf_unauthenticated_email_mandatory" ';
	echo ($wats_settings['fsf_unauthenticated_email_mandatory'] == 1) ? 'checked' : '';
	echo '> '.__('Email','WATS').' '.__('(unauthenticated user)','WATS').' </td></tr>';
	echo '<tr><td><input type="checkbox" name="fsf_unauthenticated_website_mandatory" ';
	echo ($wats_settings['fsf_unauthenticated_website_mandatory'] == 1) ? 'checked' : '';
	echo '> '.__('Website','WATS').' '.__('(unauthenticated user)','WATS').' </td></tr>';
	echo '<tr><td><input type="checkbox" name="fsf_unauthenticated_title_mandatory" ';
	echo ($wats_settings['fsf_unauthenticated_title_mandatory'] == 1) ? 'checked' : '';
	echo '> '.__('Title','WATS').' </td></tr>';
	echo '<tr><td><input type="checkbox" name="fsf_unauthenticated_description_mandatory" ';
	echo ($wats_settings['fsf_unauthenticated_description_mandatory'] == 1) ? 'checked' : '';
	echo '> '.__('Description','WATS').' </td></tr>';
	echo '<tr><td>';
	wats_options_premium_only();
	echo '<div class="wats_tip" id="mandatoryfieldfrontendsub_tip">';
	echo __('Select the fields that the user will be forced to fill in order to be able to submit a ticket through the frontend.','WATS');
	echo '</div></td></tr></table><br />';
	
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("frontendsubformsubmission_tip");>'.__('Frontend submission form successfull submission','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("email_ticket_submission_tip");>'.__('Email ticket submission','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("submitformdefaultauthor_tip");>'.__('Default author for unregistered visitors tickets','WATS').' : </a></h3>';
	echo '<table class="wats-form-table"><tr><td>'.__('User','WATS').' : ';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("call_center_ticket_creation_tip");>'.__('Call center ticket creation','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo __('Check this option if you want to allow admins to create tickets on behalf of users. This will allow them to set the ticket originator while submitting a new ticket.','WATS').'</div></td></tr></table><br />';

	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("call_center_ticket_update_tip");>'.__('Call center ticket update','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	echo '<tr><td><input type="checkbox" name="call_center_ticket_update"';
	if ($wats_settings['call_center_ticket_update'] == 1)
		echo ' checked';
	echo '> '.__('Allow admins to update a ticket on behalf of any user','WATS').'</td></tr><tr><td>';
	echo '<div class="wats_tip" id="call_center_ticket_update_tip">';
	echo __('Check this option if you want to allow admins to update tickets on behalf of any user.','WATS').'</div></td></tr></table><br />';
...
function wats_options_manage_ticket_display_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("group1_tip");>'.__('Ticket numerotation','WATS').' :</a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("wats_home_display_tip");>'.__('Tickets display','WATS').' :</a></h3>';
	echo '<table class="wats-form-table">';
	echo '<tr><td><input type="checkbox" name="homedisplay"';
	if ($wats_settings['wats_home_display'] == 1)
		echo ' checked';
	echo '> '.__('Include tickets on homepage with posts','WATS').'</td></tr><tr><td>';
	echo '<div class="wats_tip" id="wats_home_display_tip">';
	echo __('Check this option if you want to display tickets on your homepage with regular posts. If the option is unchecked only posts will be displayed.','WATS').'</div></td></tr></table><br />';
	
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("group2_tip");>'.__('Tickets visibility','WATS').' :</a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '>'.__('Only the ticket creator and admins can see tickets','WATS').'</td></tr>';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("comment_menuitem_visibility_tip");>'.__('Comments visibility','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo __('Check the first option if you want to prevent users without comment moderation capability to browse the comments list page (on this page, they could see updates on all tickets).','WATS').'<br />';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("template_selector_tip");>'.__('Template selector','WATS').' :</a></h3>';
	echo '<table class="wats-form-table">';
...
function wats_options_manage_ticket_listing_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("user_selector_format_tip");>'.__('User selector format','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("user_selector_order_tip");>'.__('User selector order','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("filter_ticket_listing_tip");>'.__('Ticket author user meta key selector for ticket listing filtering','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("meta_column_ticket_listing_tip");>'.__('Ticket author user meta key column for tickets listing table','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("ticket_listing_active_columns_tip");>'.__('Active columns in ticket listing table','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	$x = 0;
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("ticket_listing_default_query_list_tip");>'.__('List of queries','WATS').' : </a></h3>';
	echo '<div id="divticketfontendqueriestable">'.wats_options_display_frontend_queries_table().'</div>';
	wats_options_premium_only();
	echo '<br /><input type="submit" class="button-primary" id="idsupqueries" value="'.__('Remove selected queries','WATS').'" /><div id="resultsupqueries"></div><br />';
	echo '<table class="wats-form-table">';
	
	echo '<tr><td>'.__('Query name','WATS').' : ';
	echo '<input type="text" size="30" name="wats_input_ticket_query_name_tl_queries" id="wats_input_ticket_query_name_tl_queries" value="" /></td></tr>';
	
	echo '<tr><td>'.__('Ticket type','WATS').' : ';
	echo '<select name="wats_select_ticket_type_tl_queries" id="wats_select_ticket_type_tl_queries" class="wats_select">';
	echo '<option value="0">'.esc_html__('Any','WATS').'</option>';
	if (is_array($wats_ticket_type))
	foreach ($wats_ticket_type as $key => $value)
		echo '<option value="'.$key.'">'.esc_html__($value,'WATS').'</option>';
	...
	echo '<select name="wats_select_ticket_priority_tl_queries" id="wats_select_ticket_priority_tl_queries" class="wats_select">';
	echo '<option value="0">'.esc_html__('Any','WATS').'</option>';
	if (is_array($wats_ticket_priority))
	foreach ($wats_ticket_priority as $key => $value)
		echo '<option value="'.$key.'">'.esc_html__($value,'WATS').'</option>';
	...
	echo '<select name="wats_select_ticket_status_operator_tl_queries" id="wats_select_ticket_status_operator_tl_queries" class="wats_select">';
	echo '<option value="0">==</option>';
	echo '<option value="1">!=</option>';
	echo '</select> ';
	echo '<select name="wats_select_ticket_status_tl_queries" id="wats_select_ticket_status_tl_queries" class="wats_select">';
	echo '<option value="0">'.esc_html__('Any','WATS').'</option>';
	if (is_array($wats_ticket_status))
	foreach ($wats_ticket_status as $key => $value)
		echo '<option value="'.$key.'">'.esc_html__($value,'WATS').'</option>';
	...
	echo '<select name="wats_select_ticket_product_tl_queries" id="wats_select_ticket_product_tl_queries" class="wats_select">';
	echo '<option value="0">'.esc_html__('Any','WATS').'</option>';
	if (is_array($wats_ticket_product))
	foreach ($wats_ticket_product as $key => $value)
		echo '<option value="'.$key.'">'.esc_html__($value,'WATS').'</option>';
	...
	echo '<select name="wats_select_ticket_author_tl_queries" id="wats_select_ticket_author_tl_queries" class="wats_select">';
	echo wats_fill_drop_down_no_echo(array(__('Any','WATS'),__('Current user','WATS')),'');
	echo '</select></td></tr>';
	
	echo '<tr><td>'.__('Ticket owner','WATS').' : ';
	echo '<select name="wats_select_ticket_owner_tl_queries" id="wats_select_ticket_owner_tl_queries" class="wats_select">';
	echo wats_fill_drop_down_no_echo(array(__('Any','WATS'),__('None','WATS'),__('Current user','WATS')),'');
	echo '</select></td></tr>';
	
	echo '</table><br />';
	echo '<input type="submit" id="idaddquery" value="'.__('Add this query','WATS').'" class="button-primary" /><div id="resultaddquery"></div>';
	echo '<br /><br /><div class="wats_tip" id="ticket_listing_default_query_list_tip">';
	echo __('Create queries that you can then include in the frontend ticket listing shortcode by adding the Query ID at the end of the shortcode call.','WATS').'</div><br />';
	
	$wats_default_ticket_listing_default_query = $wats_settings['wats_default_ticket_listing_default_query'];
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("ticket_listing_default_query_tip");>'.__('Default query','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	echo '<tr><td>'.__('Ticket type','WATS').' : ';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("ticket_listing_display_tip");>'.__('Ticket listing display','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	echo '<tr><td><input type="checkbox" name="display_list_not_authenticated"';
	if ($wats_settings['display_list_not_authenticated'] == 1)
		echo ' checked';
	echo '> '.__('Display empty ticket list for non-authenticated users beyond the login form','WATS').'</td></tr>';
	echo '<tr><td><input type="checkbox" name="display_list_description_separate_column"';
	if ($wats_settings['display_list_description_separate_column'] == 1)
		echo ' checked';
	echo '> '.__('Display description on its own column in the exported file rather than a dedicated row','WATS').'</td></tr><tr><td>';
	wats_options_premium_only();
	echo '<div class="wats_tip" id="ticket_listing_display_tip">';
	echo __('Check this option if you want to allow non-authenticated users to view empty ticket listing when the ticket visibility requires an authentication.','WATS').
	' '.__('The second option allows to sort the XML file properply when the description is enabled.','WATS').
	'</div></td></tr></table><br />';
...
function wats_options_manage_ticket_keys_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("tickets_tagging_tip");>'.__('Tickets tagging','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("ticket_keys_selection_tip");>'.__('Ticket keys','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("tickets_wats_default_keys_management_tip");>'.__('Default keys management','WATS').' : </a></h3>';
	echo '<div id="divticketdefaultfieldstable">'.wats_options_display_default_keys_table().'</div>';
	echo '<br /><div id="resulteditdefaultfields"></div><div class="wats_tip" id="tickets_wats_default_keys_management_tip">';
	echo __('Set the visibility options for each area according to your needs.','WATS').'<br /></div><br />';
	
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("tickets_custom_fields_tip");>'.__('Custom fields','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("tickets_wats_custom_fields_tip");>'.__('Custom fields management','WATS').' : </a></h3>';
	...
	echo __('Add custom fields and set the visibility options for each area according to your needs.','WATS').'<br /><br />';
	echo __('Warning : when you edit an existing entry if you modify the meta key value, this will break existing mapping in the DB for existing tickets so make sure you understand this while modifying this.','WATS').'<br /><br />';
	echo __('If you need to reorder the custom fields to change the display order, just modify the first column entry (ID) and replace the number with the target number where you would like to see the custom field. Items will then be shifted and the complete list will be reindexed.','WATS').'</div><br />';

	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("tickets_wats_custom_fields_selector_tip");>'.__('Custom fields drop down selector and radio button values','WATS').' : </a></h3>';
	$wats_ticket_custom_field_values = $wats_settings['wats_ticket_custom_fields'];
	...
	echo '</select><br /><br />';
	echo '<div id="custom_fields_selector_values_table_div">'.__('Please select a custom field to customize selector and radio button values','WATS').'</div><br />';
	...
	echo __('Add custom values to each custom field (drop down selector and radio button) according to your needs.','WATS').'</div><br />';
...
function wats_options_manage_ticket_assign_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("group3_tip");>'.__('Tickets assignment','WATS').' :</a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("group4_tip");>'.__('Target users for tickets assignment','WATS').' :</a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("ticket_assign_role_tip");>'.__('Roles with ticket assignment capability','WATS').' : </a></h3>';
	echo '<table class="wats-form-table"><tr><td>';
	$roles = get_editable_roles();
	foreach ($roles AS $role)
	{
		echo '<input type="checkbox" name="ticket_assignment_'.strtolower(before_last_bar($role['name'])).'"';
		if (isset($wats_settings['ticket_assignment_'.strtolower(before_last_bar($role['name']))]) && $wats_settings['ticket_assignment_'.strtolower(before_last_bar($role['name']))] == 1)
			echo ' checked';
...
function wats_options_manage_user_profile_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("user_profile_administration_tip");>'.__('User profile administration','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
	...
	echo __('- Email option : check this option if you want to prevent regular users from modifying their email address under the user profile page. If checked, only administrators can update it.','WATS').'<br />';
	echo __('- Expiration option : check this option to allow administrators to set an account expiration date for each user. They won\'t be able to authenticate after the expiration date.','WATS').'<br />';
...
function wats_options_manage_troubleshoot_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("drop_down_user_selector_format_tip");>'.__('Scalability options','WATS').' : </a></h3>';
	echo '<table class="wats-form-table">';
...
function wats_options_manage_stats_options()
{
	...
	echo '<h3><a style="cursor:pointer;" title="'.__('Click to get help!', 'WATS').'" onclick=javascript:wats_invert_visibility("dashboard_stats_widget_level_tip");>'.__('Global statistics dashboard widget roles visibility','WATS').' : </a></h3>';
	echo '<table class="wats-form-table"><tr><td>';
	$roles = get_editable_roles();
	foreach ($roles AS $role)
	{
		echo '<input type="checkbox" name="dashboard_stats_widget_'.strtolower(before_last_bar($role['name'])).'"';
		if (isset($wats_settings['dashboard_stats_widget_'.strtolower(before_last_bar($role['name']))]) && $wats_settings['dashboard_stats_widget_'.strtolower(before_last_bar($role['name']))] == 1)
			echo ' checked';
		echo '> '.translate_user_role($role['name']).'<br />';
	}
	echo '</td></tr><tr><td>';
...
function wats_options_manage_home_options()
{
	...
	echo __('Just select a menu item in the right sidebar widget and set the options according to your needs.','WATS').'<br /><br />';
	...
	if (WATS_PREMIUM == false)
	{
		...
		echo __(' The Premium release comes with one year of Premium support from WATS lead developer and one year of Premium release updates.','WATS');
		...
		echo '<li>'.__('Submit tickets through the admin side (for unregistered users via the shared guest user feature)','WATS').'</li>';
		...
		echo '<br /><div class="wats-premium-feature"><h3><a href="http://www.ticket-system.net/order-now/">'.__("By upgrading to the premium release, you'll benefit from the following additional features :",'WATS').'</a></h3>';
		...
		echo '<li>'.__('Attach files to the tickets in the frontend (registered users only)','WATS').'</li>';
		echo '<li>'.__('HTML WYSIWYG editor for ticket submission and ticket update in the frontend','WATS').'</li>';
		...
		echo '<br /><H3>'.__('Also, the premium release comes with 1 year of premium updates and 1 year of support from WATS lead developer!','WATS').'</h3></div>';
		...
		echo sprintf(__('For the detailed features list please check out %s the feature list page','WATS'),'<a href="http://www.ticket-system.net/features/"> ').'</a>.';
	}
...
function wats_options_admin_menu()
{
	...
	if (isset($_POST['save']))
	{
		check_admin_referer('update-wats-options','wats_nonce');
		...
		if ($submenu == 'notifications')
		{
			...
			$wats_settings['ticket_notification_custom_list'] = intval($_POST['ticket_notification_custom_list']);
		}
		else if ($submenu == 'ticket-submission')
		{
			...
			$wats_settings['ms_mail_password'] = strlen(stripslashes($_POST['ms_mail_password'])) ? esc_html(stripslashes($_POST['ms_mail_password'])) : 'password';
			...
			$wats_settings['fsf_unauthenticated_name_mandatory'] = isset($_POST['fsf_unauthenticated_name_mandatory']) ? 1 : 0;
			$wats_settings['fsf_unauthenticated_email_mandatory'] = isset($_POST['fsf_unauthenticated_email_mandatory']) ? 1 : 0;
			$wats_settings['fsf_unauthenticated_website_mandatory'] = isset($_POST['fsf_unauthenticated_website_mandatory']) ? 1 : 0;
			$wats_settings['fsf_unauthenticated_title_mandatory'] = isset($_POST['fsf_unauthenticated_title_mandatory']) ? 1 : 0;
			$wats_settings['fsf_unauthenticated_description_mandatory'] = isset($_POST['fsf_unauthenticated_description_mandatory']) ? 1 : 0;
			$wats_settings['fsf_enable_tinymce'] = isset($_POST['fsf_enable_tinymce']) ? 1 : 0;
			$wats_settings['call_center_ticket_update'] = isset($_POST['call_center_ticket_update']) ? 1 : 0;
			$wats_settings['ftuf_enable_tinymce'] = isset($_POST['ftuf_enable_tinymce']) ? 1 : 0;
		}
		...
		else if ($submenu == 'ticket-listing')
		{
			...
			$wats_settings['display_list_description_separate_column'] = isset($_POST['display_list_description_separate_column']) ? 1 : 0;
		}
		else if ($submenu == 'ticket-keys')
		{
			...
			if (isset($_POST['group_default_custom_selector']) && isset($_POST['wats_custom_fields_selector']))
			{
				...
				if (isset($wats_ticket_custom_field_values[$_POST['wats_custom_fields_selector']]))
				{
					...
					if (isset($table['type']) && ($table['type'] == 1 || $table['type'] == 3) && isset($table['values']))
					{
						$values = $table['values'];
		...
		else if ($submenu == 'ticket-assign')
		{
			...
			foreach ($roles AS $role)
			{
				$rolename = strtolower(before_last_bar($role['name']));
	...
	wp_nonce_field('update-wats-options','wats_nonce');
