...
function wats_list_tickets_args($args)
{
	...
	{
		$output = __('The frontend ticket listing feature is only available in the premium release. Don\'t hesitate to <a href="http://www.ticket-system.net/order-now/">order the ticket system now</a>!','WATS');
	}
...
function wats_ticket_submit_form()
{
	...
	if (WATS_PREMIUM == false)
	{
		$output = __('The frontend submission form feature is only available in the premium release. Don\'t hesitate to <a href="http://www.ticket-system.net/order-now/">buy the helpdesk solution now</a>!','WATS');
	}
	...
	if (is_singular() && wats_is_ticket($wp_query->post) == true)
	{
		if (wats_check_visibility_rights())
		{
			if ($wats_settings['template_selector'] == 0)
			{
				...
				add_filter('the_content','wats_single_ticket_content_filter',99,1);
			}
			else
			{
...
function wats_single_ticket_content_filter($content)
{
	...
	if (wats_check_visibility_rights())
	{
		...
		if ($wats_settings['ticket_priority_key_enabled'] == 1 && ($wats_settings['wats_ticket_default_keys'][1]['ftdt'] == 1 || ($wats_settings['wats_ticket_default_keys'][1]['ftdt'] == 2 && current_user_can('administrator'))))
		...
		if ($wats_settings['ticket_status_key_enabled'] == 1 && ($wats_settings['wats_ticket_default_keys'][2]['ftdt'] == 1 || ($wats_settings['wats_ticket_default_keys'][2]['ftdt'] == 2 && current_user_can('administrator'))))
		...
		if ($wats_settings['ticket_type_key_enabled'] == 1 && ($wats_settings['wats_ticket_default_keys'][0]['ftdt'] == 1 || ($wats_settings['wats_ticket_default_keys'][0]['ftdt'] == 2 && current_user_can('administrator'))))
		...
		if ($wats_settings['ticket_product_key_enabled'] == 1 && ($wats_settings['wats_ticket_default_keys'][3]['ftdt'] == 1 || ($wats_settings['wats_ticket_default_keys'][3]['ftdt'] == 2 && current_user_can('administrator'))))
		...
		if ($attachments)
		{
			$after_content .= __('List of files attached to the ticket','WATS').' :';
			$after_content .= '<table class="wats_attachment_table" cellspacing="0" style="text-align:center;margin-top:10px;">';
			$after_content .= '<thead><tr class="thead"><th class="header" scope="col">'.__('Filename','WATS').'</th><th>'.__('Owner','WATS').'</th><th>'.__('Date','WATS').'</th><th>'.__('Size','WATS').'</th></tr></thead>';
			foreach ($attachments as $attachment)
			{
				$url = wp_get_attachment_url($attachment->ID);
				$title = basename(get_attached_file($attachment->ID));
				$size = size_format(filesize(get_attached_file($attachment->ID)));
				$author = get_the_author_meta('display_name',$attachment->post_author);
				$after_content .= '<tr><td><a href="'.$url.'">'.$title.'</a></td><td>'.$author.'</td><td>'.$attachment->post_date.'</td><td>'.$size.'</td></tr>';
			}
			$after_content .= '</table>';
		}
		
		$content = $output.$content.$after_content;
...
function wats_comments_array($comments,$post_id)
{
	...
	if (!current_user_can('administrator'))
	{
		...
		{
			if (in_array($comment->comment_ID,$comment_array,true))
			{
				unset($comments[$key]);
			}
			else
			{
				$comment_array[] = $comment->comment_ID;
